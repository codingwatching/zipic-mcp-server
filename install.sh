...
# Zipic MCP Server Installation Script
# https://github.com/okooo5km/zipic-mcp-server
...
echo -e "${BLUE}Installing Zipic MCP Server...${NC}"
...
if ! curl -L "https://github.com/okooo5km/zipic-mcp-server/releases/latest/download/zipic-mcp-server.tar.gz" | tar xz -C "$HOME/.local/bin"; then
  echo -e "${RED}Error: Failed to download or extract the binary.${NC}"
  exit 1
fi

chmod +x "$HOME/.local/bin/zipic-mcp-server"

# Verify installation
if [ -x "$HOME/.local/bin/zipic-mcp-server" ]; then
  echo -e "${GREEN}✅ Installation completed successfully!${NC}"
  
  # Check if executable is in PATH
  if command -v zipic-mcp-server >/dev/null 2>&1; then
    echo -e "${GREEN}zipic-mcp-server is in your PATH and ready to use.${NC}"
    echo -e "${BLUE}Version information:${NC}"
    zipic-mcp-server --version
  else
    echo -e "${YELLOW}Note: zipic-mcp-server is installed but may not be in your current PATH.${NC}"
    echo -e "${YELLOW}Run the following command to use it immediately:${NC}"
    echo -e "${BLUE}$HOME/.local/bin/zipic-mcp-server --version${NC}"
  ...
  echo -e "${GREEN}zipic-mcp-server --help${NC}"
else
